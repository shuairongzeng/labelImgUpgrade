...
### 数据集大小分类（防过拟合优化版）
- **极小数据集**（<100张）：基础轮数 80（保守策略，防止过拟合）
- **小数据集**（100-800张）：基础轮数 100（谨慎策略）
- **中等数据集**（800-3000张）：基础轮数 120
- **大数据集**（3000-10000张）：基础轮数 100
- **超大数据集**（>10000张）：基础轮数 80
...
### 小数据集特别注意事项
智能计算器已针对小数据集过拟合问题进行优化：

#### 数据集大小 < 100张
- **轮数策略**：采用保守的80轮基础轮数
- **必须措施**：
  - ✅ 启用Early Stopping (patience=10-20)
  - ✅ 使用数据增强技术
  - ✅ 选择较小模型(yolov8n)
  - ✅ 每10轮检查验证集性能
